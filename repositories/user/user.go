...
func NewUserRepository(db *gorm.DB) IUserRepository {
	return &UserRepository{db: db} //& ambil alamat memori dari struct yang baru dibuat.
									// Hasilnya tipe *UserRepository (pointer).
}

func (r *UserRepository) Register(ctx context.Context, req *dto.RegisterRequest) (*models.User, error) { 
	// UserRepository : pakai pointer untuk menghindari copy struct yang besar
	user := models.User {
		UUID: uuid.New(),
		Name: req.Name,
